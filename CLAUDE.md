...
**BurpeePacer** — a web app for a fitness community (Burpee Pacers) that runs structured burpee workout sessions. Users are assigned to Beginner or Advanced tracks with level-based rep goals. The app provides a live workout timer, progress tracking, admin analytics, and subscription gating.
...
├── shared/               # Shared TypeScript types/logic (workoutTimer.ts)
...
### Shared Logic (`shared/workoutTimer.ts`)

- `WorkoutMode` — `"N" | "C" | "H"`
- `buildWorkoutTimerConfig()` — builds mode/goal config per user level and tier
...
- `WorkoutTimer.tsx` — live timer UI; supports all three modes; Hybrid shows phase label, phase countdown, total-remaining caption, per-phase rep counter
...
- `ProGate.tsx` — gates Pro features; non-Pro users see upgrade prompt
- CSV export and advanced analytics are Pro-only
